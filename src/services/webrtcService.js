...
class WebRTCService {
  ...
  unmuteParticipant(targetId) {
    if (this.socket) this.socket.emit('force-unmute', { targetId });
  }
  ...
  cameraOnParticipant(targetId) {
    if (this.socket) this.socket.emit('force-camera-on', { targetId });
  }
