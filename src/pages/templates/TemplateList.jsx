...
import { Plus, Download, Upload, Search, Filter, FileCode, Edit, Trash2, LayoutTemplate, Activity, ScanLine, ScanFace } from 'lucide-react';
import Badge from '../../components/common/Badge';

const mockTemplates = [
  { id: 'TPL-001', name: 'CT Brain Routine Report', category: 'CT', author: 'Dr. Sarah Connor', updated: 'Oct 26, 2023', status: 'Active' },
  { id: 'TPL-002', name: 'MRI Spine with Contrast', category: 'MRI', author: 'Dr. Emily Chen', updated: 'Oct 25, 2023', status: 'Active' },
  { id: 'TPL-003', name: 'Ultrasound Abdomen Complete', category: 'Ultrasound', author: 'Dr. John Doe', updated: 'Oct 24, 2023', status: 'Draft' },
  { id: 'TPL-004', name: 'Doppler Lower Limb Venous', category: 'Doppler', author: 'Dr. Samuel Reefath', updated: 'Oct 20, 2023', status: 'Archived' },
];
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-text-main">Reporting Templates</h1>
          <p className="text-sm text-text-muted mt-1">Manage standardized radiology reporting structures for training and clinical use.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          >
            <Plus className="w-4 h-4 text-brand-accent" /> Add Template
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          { label: 'Total Templates', value: '42', icon: LayoutTemplate, color: 'text-brand-primary', bg: 'bg-brand-primary/10' },
          { label: 'Active', value: '38', icon: Activity, color: 'text-emerald-600', bg: 'bg-emerald-100' },
          { label: 'CT Templates', value: '15', icon: ScanLine, color: 'text-blue-600', bg: 'bg-blue-100' },
          { label: 'MRI Templates', value: '12', icon: ScanFace, color: 'text-purple-600', bg: 'bg-purple-100' },
          { label: 'US Templates', value: '10', icon: LayoutTemplate, color: 'text-amber-600', bg: 'bg-amber-100' },
          { label: 'Doppler', value: '5', icon: Activity, color: 'text-rose-600', bg: 'bg-rose-100' },
        ].map(stat => (
                ...
                <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-brand-primary/10 text-brand-primary flex items-center justify-center shrink-0 cursor-pointer" onClick={() => navigate(`/templates/${item.id}`)}>
                  ...
                  <td className="px-6 py-4 text-right relative">
                    <div className="flex items-center justify-end gap-1">
                      <button className="p-1.5 hover:bg-gray-100 rounded text-gray-400 hover:text-brand-primary" title="Edit Template" onClick={() => navigate(`/templates/${item.id}`)}>
                        <Edit className="w-4 h-4" />
                      </button>
                      <button className="p-1.5 hover:bg-red-50 rounded text-gray-400 hover:text-red-600" title="Delete Template">
