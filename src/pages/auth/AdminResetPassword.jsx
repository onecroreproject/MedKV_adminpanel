...
import { validateResetToken, resetPasswordAdmin } from '../../services/authService';
...
export default function AdminResetPassword() {
  ...
  const onSubmit = async (data) => {
    try {
      setErrorMsg('');
      setIsLoading(true);
      await resetPasswordAdmin(token, data.password);
      setIsLoading(false);
      setIsSubmitted(true);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 border border-slate-100 text-center">
          ...
          <Link to="/admin/forgot-password" className="inline-block bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition-colors">
            Request New Link
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 border border-slate-100">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2">Enter your new admin password below.</p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-lg border ${errors.confirmPassword ? 'border-red-500' : 'border-slate-300'} focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none transition-colors bg-slate-50`}
                placeholder="••••••••"
              />
