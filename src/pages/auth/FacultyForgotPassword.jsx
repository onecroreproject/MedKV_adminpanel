...
import { forgotPasswordFaculty } from '../../services/authService';
...
export default function FacultyForgotPassword() {
  ...
  const onSubmit = async (data) => {
    try {
      setIsLoading(true);
      setErrorMsg('');
      await forgotPasswordFaculty(data.email);
      setIsLoading(false);
      setIsSubmitted(true);
    } catch (err) {
      setIsLoading(false);
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 border border-slate-100">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-slate-800">Faculty Password Reset</h2>
        ...
        {isSubmitted ? (
          <div className="text-center p-6 bg-purple-50 rounded-lg border border-purple-200">
            ...
            <h3 className="text-lg font-medium text-purple-900 mb-2">Check your email</h3>
            <p className="text-purple-700 text-sm">We have sent password reset instructions to your email address.</p>
